...
const validationErrorsObject: {[key:string]: ValidationError} = {
  ...
  minLowercase: {
    ...
    message: `Contraseña debe tener al menos ${validations.minLowercase} caracter${validations.minLowercase > 1 ? "es" : ""} singular${validations.minLowercase > 1 ? "es" : ""}`,
    field: 'password'
  },
  minUppercase: {
    code: validationErrorCodes.PASSWORD_DOESNT_CONTAIN_MIN_UPPERCASE,
    message: `Contraseña debe tener al menos ${validations.minUppercase} caracter${validations.minUppercase > 1 ? "es" : ""} singular${validations.minUppercase > 1 ? "es" : ""}`,
    field: 'password'
  },
  minNumbers: {
    code: validationErrorCodes.PASSWORD_DOESNT_CONTAIN_MIN_NUMBERS,
    message: `Contraseña debe tener al menos ${validations.minNumbers} caracter${validations.minNumbers > 1 ? "es" : ""} numérico${validations.minNumbers > 1 ? "s" : ""}`,
    field: 'password'
  },
  minSymbols: {
    code: validationErrorCodes.PASSWORD_DOESNT_CONTAIN_MIN_SYMBOLS,
    message: `Contraseña debe tener al menos ${validations.minSymbols} caracter${validations.minSymbols > 1 ? "es" : ""} especial${validations.minSymbols > 1 ? "es" : ""}`,
    field: 'password'
  },
}
...
export default function password(password: string): Validation {
  ...
  Object.keys(validations).forEach((validation: string) => {
    if(analysis[validations_analysis[validation]] < validations[validation]) errors.push(validationErrorsObject[validation])
  })
