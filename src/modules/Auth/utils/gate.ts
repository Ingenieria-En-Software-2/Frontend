...
export const getRouteConfig = (path: string): routeConfigInterface => {
  const routeConfig = routesConfig.privateRoutes.find((routeConfig) => routeConfig.routeRegExp.test(path));
...
export const getDevelopRouteConfig = (path: string): routeConfigInterface => {
  const developRouteConfig = developRoutesConfig.developRoutes.find((developRoutesConfig) =>
...
export const isAllowedRoute = (_user: UserInterface, _route_config: routeConfigInterface): boolean => {
  //TODO: Check route configuration and user role and grant or revoke access
  return true;
};

export const isDeveloperRoute = (_user: UserInterface, _route_config: routeConfigInterface): boolean => {
